...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_min_tft_price(origin: OriginFor<T>, price: u32) -> DispatchResultWithPostInfo {
			...
			ensure!(price < MaxTftPrice::<T>::get(), Error::<T>::MinPriceAboveMaxPriceError);
		...
		pub fn set_max_tft_price(origin: OriginFor<T>, price: u32) -> DispatchResultWithPostInfo {
			T::RestrictedOrigin::ensure_origin(origin)?;
			ensure!(price > MinTftPrice::<T>::get(), Error::<T>::MaxPriceBelowMinPriceError);
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		fn offchain_worker(block_number: T::BlockNumber) {
			match Self::offchain_signed_tx(block_number) {
				Ok(_) => log::info!("tft price offchain worker done."),
	...
	impl<T: Config> Default for GenesisConfig<T> {
		fn default() -> Self {
			Self { min_tft_price: 10, max_tft_price: 1000, _data: PhantomData }
		}
	}
...
impl<T: Config> Pallet<T> {
	...
	fn fetch_price() -> Result<u32, http::Error> {
		...
		let tft_usd =
			(U32F32::from_num(price) / U32F32::from_num(DST_AMOUNT)).round().to_num::<u32>();
	...
	fn offchain_signed_tx(block_number: T::BlockNumber) -> Result<(), Error<T>> {
		...
		let result =
			signer.send_signed_transaction(|_acct| Call::set_prices { price, block_number });
		...
		log::info!("No local account available for tft price worker");
		return Err(<Error<T>>::OffchainSignedTxError);
	}
	...
	pub fn parse_lowest_price_from_request(price_str: &str) -> Option<u32> {
		...
		let prices: Vec<U32F32> = records_array
			...
			.map(|x| if let Some(p) = x { p } else { U32F32::from_num(f32::NAN) })
			.collect();
