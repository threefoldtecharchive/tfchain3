...
impl<T: Config> Pallet<T> {
	...
	pub fn _create_rent_contract(
		...
		let node = pallet_tfgrid::Nodes::<T>::get(node_id).ok_or(Error::<T>::NodeNotExists)?;
		ensure!(pallet_tfgrid::Farms::<T>::contains_key(node.farm_id), Error::<T>::FarmNotExists);
	...
	pub fn _update_node_contract(
		...
		ensure!(!is_grace_state, Error::<T>::CannotUpdateContractInGraceState);
	...
	pub fn _cancel_contract(
		...
		if matches!(&contract.contract_type, types::ContractData::RentContract(_)) {
			...
			ensure!(active_node_contracts.len() == 0, Error::<T>::NodeHasActiveContracts);
		}
	...
	pub fn _compute_reports(
		...
		for report in &reports {
			...
			let node_contract = Self::get_node_contract(&contract)?;
			ensure!(node_contract.node_id == node_id, Error::<T>::NodeNotAuthorizedToComputeReport);
	...
	fn _distribute_cultivation_rewards(
		...
		if let Some(provider_id) = contract.solution_provider_id {
			if let Some(solution_provider) = SolutionProviders::<T>::get(provider_id) {
				let total_take: u8 =
					solution_provider.providers.iter().map(|provider| provider.take).sum();
		...
		log::debug!("Burning: {:?} from contract twin {:?}", amount_to_burn, &twin.account_id);
	...
	pub fn insert_contract_to_bill(contract_id: u64) {
		...
		if !contracts.contains(&contract_id) {
			...
			ContractsToBillAt::<T>::insert(index, &contracts);
			log::debug!("Insert contracts: {:?}, to be billed at index {:?}", contracts, index);
		}
	}
		...
		for i in 0..farm.public_ips.len() {
			...
			if farm.public_ips[i].contract_id == 0 {
				...
				ips.try_push(ip).or_else(|_| {
					return Err(DispatchErrorWithPostInfo::from(Error::<T>::FailedToReserveIP));
				})?;
			}
		}
	...
	pub fn _free_ip(
		...
		Self::deposit_event(Event::IPsFreed { contract_id, public_ips });
	...
	pub fn get_node_contract(
		...
		match contract.contract_type.clone() {
			...
			_ => return Err(DispatchErrorWithPostInfo::from(Error::<T>::InvalidContractType)),
		}
	}
	...
	pub fn _approve_solution_provider(
		...
		if let Some(mut solution_provider) = SolutionProviders::<T>::get(solution_provider_id) {
			...
			Self::deposit_event(Event::SolutionProviderApproved(solution_provider_id, approve));
		}
	...
	pub fn validate_solution_provider(
		...
		if let Some(provider_id) = solution_provider_id {
			...
			if let Some(solution_provider) = SolutionProviders::<T>::get(provider_id) {
				ensure!(solution_provider.approved, Error::<T>::SolutionProviderNotApproved);
				return Ok(().into());
			}
		}
		Ok(().into())
	...
	pub fn _service_contract_create(
		...
		ensure!(service_twin_id != consumer_twin_id, Error::<T>::ServiceContractCreationNotAllowed,);
	...
	pub fn _service_contract_set_metadata(
		...
		// Only allow to modify metadata if contract still not approved by both parties
			...
			!matches!(service_contract.state, types::ServiceContractState::ApprovedByBoth),
	...
	pub fn _service_contract_set_fees(
		...
		ensure!(twin_id == service_contract.service_twin_id, Error::<T>::TwinNotAuthorized,);
	...
	pub fn _service_contract_approve(
		...
		// Allow to approve contract only if agreement is ready
			...
			matches!(service_contract.state, types::ServiceContractState::AgreementReady),
		...
		if twin_id == service_contract.service_twin_id {
			...
			return Err(DispatchErrorWithPostInfo::from(Error::<T>::TwinNotAuthorized));
		}
	...
	pub fn _service_contract_cancel(
		...
		Self::deposit_event(Event::ServiceContractCanceled { service_contract_id, cause });

		log::debug!("successfully removed service contract with id {:?}", service_contract_id,);
	...
	pub fn _service_contract_bill(
		...
		let service_contract_bill =
			types::ServiceContractBill { variable_amount, window, metadata: bill_metadata };
	...
	pub fn _change_billing_frequency(frequency: u64) -> DispatchResultWithPostInfo {
		...
		ensure!(frequency > billing_frequency, Error::<T>::CanOnlyIncreaseFrequency);
